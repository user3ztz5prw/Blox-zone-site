...
function render(){
 ...
 if(f==="low")list=list.filter(p=>p.price<=50);
 if(f==="mid")list=list.filter(p=>p.price>50&&p.price<=100);
 if(f==="high")list=list.filter(p=>p.price>100);
...
function updateCart(){
 ...
 `).join("")
 :`<p class="hero-text">Seu carrinho está vazio.</p>`;
